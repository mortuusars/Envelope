...
public class AddressTagApplicationRecipe extends CustomRecipe {
    public AddressTagApplicationRecipe(CraftingBookCategory category) {
        super(category);
    }
    ...
    public @NotNull ItemStack assemble(CraftingInput input, HolderLookup.Provider registries) {
        ...
        for (int i = 0; i < input.size(); i++) {
            ...
            if (stack.is(Envelope.Tags.Items.MAILABLE)) {
                target = stack;
            } else if (stack.getItem() instanceof AddressTagItem) {
                tag = stack;
            }
        }
