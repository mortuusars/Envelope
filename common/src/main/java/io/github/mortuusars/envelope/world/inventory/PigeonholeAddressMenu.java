...
public class PigeonholeAddressMenu extends AbstractContainerMenu {
    ...
    protected PigeonholeAddressMenu(MenuType<?> type, int containerId, Inventory playerInventory, BlockPos pos, String suggestedAddress) {
    ...
    public PigeonholeAddressMenu(int containerId, Inventory playerInventory, BlockPos pos, String suggestedAddress) {
        this(Envelope.MenuTypes.PIGEONHOLE_ADDRESS.get(), containerId, playerInventory, pos, suggestedAddress);
    }

    public static PigeonholeAddressMenu fromNetwork(int containerId, Inventory playerInventory, RegistryFriendlyByteBuf buffer) {
        return new PigeonholeAddressMenu(containerId, playerInventory, buffer.readBlockPos(), buffer.readUtf());
    }
    ...
    public boolean stillValid(Player player) {
        return player.level().getBlockState(pos).getBlock() instanceof PigeonholeBlock
    ...
    public boolean clickMenuButton(Player player, int id) {
        if (id == APPLY_BUTTON_ID && canConfirm()) {
            if (player.level() instanceof ServerLevel
                    && getPlayer().level().getBlockEntity(pos) instanceof PigeonholeBlockEntity mailboxBlockEntity) {
