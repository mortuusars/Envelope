...
public interface OldAddress {
    OldAddress MAIL_SERVICE = new Npc("<Mail Service>", Component.translatable("address.envelope.mail_service"));

    Codec<OldAddress> CODEC = Type.CODEC.dispatch(OldAddress::type, Type::getCodec);
    StreamCodec<RegistryFriendlyByteBuf, OldAddress> STREAM_CODEC = Type.STREAM_CODEC.dispatch(OldAddress::type, Type::getStreamCodec);

    Type type();
    String id();
    Component getPresentableName();

    record Player(String name, UUID uuid) implements OldAddress {
    ...
    record Npc(String name, Optional<Component> displayName) implements OldAddress {
    ...
    record Unknown(String name) implements OldAddress {
    ...
    enum Type implements StringRepresentable {
        ...
        private final MapCodec<? extends OldAddress> codec;
        private final StreamCodec<RegistryFriendlyByteBuf, ? extends OldAddress> streamCodec;

        Type(String name, MapCodec<? extends OldAddress> codec, StreamCodec<RegistryFriendlyByteBuf, ? extends OldAddress> streamCodec) {
            this.name = name;
            this.codec = codec;
        ...
        public MapCodec<? extends OldAddress> getCodec() {
            return codec;
        }

        public StreamCodec<RegistryFriendlyByteBuf, ? extends OldAddress> getStreamCodec() {
            return streamCodec;
        }
