...
public class BuggerPackets {
    public static void sendPigeonDelivery(Pigeon pigeon) {
        if (Envelope.debug() && pigeon.level() instanceof ServerLevel) {
            Packets.sendToAllClients(new BuggerPigeonDeliveryS2CP(pigeon.getId(), Optional.ofNullable(pigeon.delivery())));
        }
    }
