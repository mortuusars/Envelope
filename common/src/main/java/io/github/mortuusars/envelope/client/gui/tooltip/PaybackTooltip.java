...
public record PaybackTooltip(Payback payback) implements ClientTooltipComponent {
    public static final ResourceLocation SLOT_SPRITE = Envelope.resource("tooltip/payback/slot");

    private static final ItemStack PAYBACK_TAG_STACK = new ItemStack(Envelope.Items.PAYBACK_TAG.get());

    @Override
    public int getWidth(Font font) {
        return 16 + (18 * payback().items().size()) + 4;
    }
    ...
    public void renderImage(Font font, int x, int y, GuiGraphics guiGraphics) {
        guiGraphics.renderFakeItem(PAYBACK_TAG_STACK, x, y + 3, 0);
