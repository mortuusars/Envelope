...
public class PigeonOnShoulderLayer<T extends Player> extends RenderLayer<T, PlayerModel<T>> {
	private final PigeonModel model;

	public PigeonOnShoulderLayer(RenderLayerParent<T, PlayerModel<T>> renderer, EntityModelSet modelSet) {
		super(renderer);
		this.model = new PigeonModel(modelSet.bakeLayer(EnvelopeModelLayers.PIGEON));
	}
		...
		EntityType.byString(compoundTag.getString("id"))
			...
			.ifPresent(
				type -> {
					...
					VertexConsumer vertexConsumer = buffer.getBuffer(this.model.renderType(PigeonRenderer.getVariantTexture(variant)));
