...
public record ServerboundPaybackTagMenuSetSlotPacket(int slotIndex, ItemStack item) implements Packet {
    public static final ResourceLocation ID = Envelope.resource("payback_tag_menu_set_slot");
    public static final Type<ServerboundPaybackTagMenuSetSlotPacket> TYPE = new Type<>(ID);

    public static final StreamCodec<RegistryFriendlyByteBuf, ServerboundPaybackTagMenuSetSlotPacket> STREAM_CODEC = StreamCodec.composite(
            ByteBufCodecs.VAR_INT, ServerboundPaybackTagMenuSetSlotPacket::slotIndex,
            ItemStack.STREAM_CODEC, ServerboundPaybackTagMenuSetSlotPacket::item,
            ServerboundPaybackTagMenuSetSlotPacket::new
    );
